...
# 3\. \*\*Lambda Consumidora\*\* processa a mensagem: busca o produto no catálogo e atualiza o registro no DynamoDB para `COMPLETED` (com dados do produto) ou `NOT\_FOUND`.
...
# &#x20;     "Resource": "arn:aws:dynamodb:SUA\_REGIAO:SEU\_ACCOUNT\_ID:table/catalogo-produtos"
...
# &#x20;     "Resource": "arn:aws:s3:::SEU\_BUCKET/images/\*"
...
# > Substitua `SUA\_REGIAO` pela região (ex: us-east-1), `SEU\_ACCOUNT\_ID` pelo ID da conta (12 dígitos) e `SEU\_BUCKET` pelo nome do seu bucket S3.
...
# &#x20;  - Na aba Código, o arquivo padrão vem como `lambda\_function.py` — \*\*delete esse arquivo\*\*
...
# &#x20;  - Mude o handler para: \*\*`app.lambda\_handler`\*\*
...
# &#x20;  - `CATALOGO\_TABLE` = `catalogo-produtos`

# &#x20;  - `IMAGES\_BUCKET` = nome do seu bucket S3

# &#x20;  - `IMAGES\_PREFIX` = `images/produtos/`
...
# const API\_URL = "URL\_DO\_SEU\_API\_GATEWAY";
...
# &#x20;      ↓ atualiza DynamoDB (COMPLETED ou NOT\_FOUND)
...
# &#x20;     "Resource": "arn:aws:sqs:SUA\_REGIAO:SEU\_ACCOUNT\_ID:pesquisa-produtos-fila"
...
# &#x20;     "Resource": "arn:aws:dynamodb:SUA\_REGIAO:SEU\_ACCOUNT\_ID:table/pesquisas-produtos"
...
# > Substitua `SUA\_REGIAO` pela região (ex: us-east-1) e `SEU\_ACCOUNT\_ID` pelo ID da conta (12 dígitos).
...
# > - O arquivo de código deve se chamar \*\*`app.py`\*\* (não `lambda\_function.py`)

# > - O handler deve ser \*\*`app.lambda\_handler`\*\* (não `lambda\_function.lambda\_handler`)
...
# &#x20; - `QUEUE\_URL` = URL da fila `pesquisa-produtos-fila` (copie do SQS → Detalhes da fila → URL)

# &#x20; - `TABLE\_NAME` = `pesquisas-produtos`
...
# 2\. Resultado: "Produto não encontrado" (status NOT\_FOUND)
